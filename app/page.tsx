...
import { aboutContent, projects, services } from "./data";
    ...
    <main>
      ...
      <section className="py-20 bg-gradient-mono-light">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {aboutContent.whatWeDo.services.slice(0, 3).map((service, index) => {
              // Define monochromatic color schemes for different services
              const colorVariations = [
                { bg: "bg-custom-light", iconColor: "text-custom-dark" },
                { bg: "bg-custom-lighter", iconColor: "text-custom-medium" },
                { bg: "bg-custom-medium", iconColor: "text-white" },
              ];

              const colors = colorVariations[index % colorVariations.length];
              
              // Get the icon component dynamically
              const IconComponent = service.icon ? LucideIcons[service.icon as keyof typeof LucideIcons] as React.ComponentType<{ className?: string }> : null;
              ...
              return (
                ...
                >
                  <motion.div
                    ...
                    transition={{ type: "spring", stiffness: 400, damping: 17 }}
                  >
                    {IconComponent && <IconComponent className={`w-8 h-8 ${colors.iconColor}`} />}
                  </motion.div>
